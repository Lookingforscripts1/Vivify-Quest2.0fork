...
void UpdateBlitEffects(float songTime) {
...
void HandleCreateCamera(rapidjson::Value const& json) {
  ...
  if (mainCam != nullptr && UnityEngine::Object::op_Implicit_bool(mainCam)) {
    w = mainCam->get_pixelWidth();
    h = mainCam->get_pixelHeight();
  }
  if (w <= 0) w = 1024;
  if (h <= 0) h = 512;
...
void ApplyCameraProperties(UnityEngine::Camera* camera, CameraPropertyData const& props) {
  if (camera == nullptr || !UnityEngine::Object::op_Implicit_bool(camera)) return;
...
void HandleSetRenderingSettings(CustomJSONData::CustomEventData* customEventData,
  ...
  if (rsVal != nullptr && rsVal->IsObject()) {
    for (auto it = rsVal->MemberBegin(); it != rsVal->MemberEnd(); ++it) {
      ...
      ParseAndApplyRenderSetting(*rsVal, key, settings, noDuration);
    }
  }
  ...
  if (qsVal != nullptr && qsVal->IsObject()) {
    for (auto it = qsVal->MemberBegin(); it != qsVal->MemberEnd(); ++it) {
      std::string key = it->name.GetString();
      ParseAndApplyQualitySetting(*qsVal, key, settings, noDuration);
    }
  }
...
void ParseAndApplyRenderSetting(rapidjson::Value const& parent, std::string const& key,
                                std::vector<RenderSettingValue>& out, bool immediate) {
  auto const& val = parent[key.c_str()];
  if (key == "fogColor" || key == "ambientLight" || key == "ambientSkyColor" ||
    ...
    if (val.IsArray()) {
      if (val.Size() > 0 && val[0].IsNumber()) {
        auto a = val.GetArray();
        UnityEngine::Color c(a[0].GetFloat(), a[1].GetFloat(), a[2].GetFloat(),
                             val.Size() > 3 ? a[3].GetFloat() : 1.0f);
        ApplyRenderSettingColor(key, c);
        if (!immediate) {
          RenderSettingValue rsv; rsv.name = key; rsv.kind = RenderSettingKind::Color; rsv.value = c; out.push_back(std::move(rsv));
        }
      } else if (!immediate) {
        auto pd = MakePointDefinition(parent, key, Tracks::ffi::WrapBaseValueType::Vec4);
        if (pd.has_value()) { 
          RenderSettingValue rsv; rsv.name = key; rsv.kind = RenderSettingKind::Color; rsv.value = std::move(pd.value()); out.push_back(std::move(rsv)); 
        }
      }
    } else if (val.IsString() && !immediate) {
      auto pd = MakePointDefinition(parent, key, Tracks::ffi::WrapBaseValueType::Vec4);
      if (pd.has_value()) { 
        RenderSettingValue rsv; rsv.name = key; rsv.kind = RenderSettingKind::Color; rsv.value = std::move(pd.value()); out.push_back(std::move(rsv)); 
      }
  ...
  } else if (!immediate && (val.IsString() || val.IsArray())) {
    auto pd = MakePointDefinition(parent, key, Tracks::ffi::WrapBaseValueType::Float);
    if (pd.has_value()) {
      RenderSettingValue rsv;
      rsv.name = key;
      rsv.kind = RenderSettingKind::Float;
      rsv.value = std::move(pd.value());
      out.push_back(std::move(rsv));
    }
  }
}
void ParseAndApplyQualitySetting(rapidjson::Value const& parent, std::string const& key,
                                 std::vector<RenderSettingValue>& out, bool immediate) {
  ParseAndApplyRenderSetting(parent, key, out, immediate);
}
void UpdateRenderSettingAnimations(float songTime) {
  ...
  for (auto read = _renderSettingAnimations.begin(); read != _renderSettingAnimations.end(); ++read) {
    ...
    for (auto const& s : read->settings) {
      ...
      } else if (s.kind == RenderSettingKind::Color && std::holds_alternative<PointDefinitionW>(s.value)) {
        auto v4 = std::get<PointDefinitionW>(s.value).InterpolateVector4(progress);
        UnityEngine::Color c(v4.x, v4.y, v4.z, v4.w);
        ApplyRenderSettingColor(s.name, c);
      }
    }
